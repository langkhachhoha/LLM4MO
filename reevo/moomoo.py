...
class ReEvoMooMoo:
    def __init__(
        ...
        self.pareto_front_obj = None
        self.pareto_front_code = None
        self.pareto_front_code_path = None
        self.init_prompt()
    ...
    def init_prompt(self) -> None:
        ...
        self.num_objectives = self.cfg.problem.num_objectives
        ...
        self.output_file = f"{self.root_dir}/problems_moo/{self.problem}/gpt.py"
    ...
    def mark_invalid_individual(self, individual: dict, traceback_msg: str) -> dict:
        ...
        individual["obj"] = [float("inf")]*self.num_objectives
    ...
    def evaluate_population(self, population: list[dict]) -> list[dict]:
        ...
        for response_id, inner_run in enumerate(inner_runs):
            ...
            # Store objective values for each individual (for multi-objective optimization)
            if traceback_msg == '':  # If execution has no error
                try:
                    objectives = extract_objectives(stdout_str)
                    individual["obj"] = objectives  
                    individual["exec_success"] = True
                except Exception as e:
                    print(e)
                    population[response_id] = self.mark_invalid_individual(population[response_id], f"Invalid std out / objective value! {e}")
            else:  
    ...
    def _run_code(self, individual: dict, response_id, debug = True) -> subprocess.Popen:
        """
        ...
        with open(individual["stdout_filepath"], 'w') as f:
            eval_file_path = f'{self.root_dir}/problems_moo/{self.problem}/eval.py' if self.problem_type != "black_box" else f'{self.root_dir}/problems_moo/{self.problem}/eval_black_box.py' 
    ...
    def update_iter(self) -> None:
        ...
        n = len(population)
        processed_objs = np.array([
            individual["obj"] for individual in population
        ])
        comp = processed_objs[None, :, :] <= processed_objs[:, None, :]
        strict = processed_objs[None, :, :] < processed_objs[:, None, :]
        dominate_matrix = np.all(comp, axis=2) & np.any(strict, axis=2)
        dominated = np.any(dominate_matrix, axis=1)
        pareto_indices = np.nonzero(~dominated)[0]
        self.pareto_front = [population[i] for i in pareto_indices]
        self.pareto_front_code = [population[i]["code"] for i in pareto_indices]
        self.pareto_front_code_path = [population[i]["code_path"] for i in pareto_indices]
        logging.info(f"Iteration {self.iteration}: Pareto Front Indices: {pareto_indices.tolist()}")
        self.pareto_front_obj = [population[i]["obj"] for i in pareto_indices]
        logging.info(f"Iteration {self.iteration}: Pareto Front Objectives: {self.pareto_front_obj}")
        ...
        # logging.info(f"Function Evals: {self.function_evals}")
        # self.iteration += 1
    def MEoH_select(self, population: list[dict]) -> list[dict]:
        """
        https://arxiv.org/pdf/2409.16867
        
        Ý tưởng tóm tắt:
        1) Lọc bỏ cá thể không hợp lệ (exec_success=False).
        2) Tính ma trận bất đồng S, trong đó S[i,j] = 1 - code_similarity_ast.
        3) Tính ma trận thống trị D, D[i,j] = 1 nếu cá thể j thống trị cá thể i.
        4) S' = S ∘ D (tích từng phần tử).
        5) Tính vector pi bằng cách lấy tổng cột của S', rồi chuẩn hóa bằng softmax.
        6) Lấy mẫu 2 * pop_size cá thể theo phân phối pi để làm cha mẹ (giống rank_select cũ).
        """

        if self.problem_type == "black_box":
            filtered_pop = [
                ind for ind in population 
                if ind["exec_success"] and ind["obj"] is not None and dominates(ind["obj"], self.seed_ind["obj"])
            ]
        else:
            filtered_pop = [
                ind for ind in population 
                if ind["exec_success"] and ind["obj"] is not None
            ]
        
        if len(filtered_pop) < 2:
            return None
        
        n = len(filtered_pop)
        codes = [ind["code"] for ind in filtered_pop]
        objs = [ind["obj"] for ind in filtered_pop]  
        
        S = np.zeros((n, n), dtype=np.float32)
        for i in range(n):
            for j in range(n):
                if i != j:
                    sim = code_similarity_ast(codes[i], codes[j])
                    S[i, j] = 1.0 - sim
        

        D = np.zeros((n, n), dtype=bool)
        objs_array = np.array(objs, dtype=np.float32)  # shape: (n, m)
        for i in range(n):
            for j in range(n):
                if i != j:
                    if np.all(objs_array[j] <= objs_array[i]) and np.any(objs_array[j] < objs_array[i]):
                        D[i, j] = True
        ...
        pi_raw = np.sum(S_prime, axis=0)  
        maxval = np.max(pi_raw)
        if maxval == 0:
            pi = np.ones(n, dtype=np.float32) / n
        else:
            exps = np.exp(pi_raw - maxval)
            pi = exps / np.sum(exps)
        ...
        while len(selected_indices) < target_size:
            trial += 1
            parents = np.random.choice(np.arange(n), size=2, replace=False, p=pi)
            if not np.allclose(objs_array[parents[0]], objs_array[parents[1]]):
                selected_indices.extend(parents)
            if trial > 1000:
                return None
        
        selected_parents = [filtered_pop[idx] for idx in selected_indices]
    ...
    def gen_short_term_reflection_prompt(self, ind1: dict, ind2: dict) -> tuple[list[dict], str, str]:
        """
        Short-term reflection before crossovering two individuals for a multi-objective problem.
        If neither individual dominates the other, the mean of the objective values is used to decide.
        """
        if ind1["obj"] == ind2["obj"]:
            print(ind1["code"], ind2["code"])
            raise ValueError("Two individuals to crossover have the same objective values!")
        
        if dominates(ind1["obj"], ind2["obj"]):
            better_ind, worse_ind = ind1, ind2
        elif dominates(ind2["obj"], ind1["obj"]):
            better_ind, worse_ind = ind2, ind1
        else:
            # Neither dominates the other; use the arithmetic mean of objective values for comparison.
            mean1 = sum(ind1["obj"]) / len(ind1["obj"])
            mean2 = sum(ind2["obj"]) / len(ind2["obj"])
            if mean1 < mean2:
                better_ind, worse_ind = ind1, ind2
            else:
                better_ind, worse_ind = ind2, ind1
    ...
    def mutate(self) -> list[dict]:
        ...
        system = self.system_generator_prompt
        func_signature1 = self.func_signature.format(version=1) 
        if not self.pareto_front or len(self.pareto_front) == 0:
            raise RuntimeError("Pareto front is empty, cannot perform mutation.")
        base_candidate = np.random.choice(self.pareto_front) if isinstance(self.pareto_front, list) else self.pareto_front[0]
        user = self.mutataion_prompt.format(
            user_generator = self.user_generator_prompt,
            reflection = self.long_term_reflection_str + self.external_knowledge,
            func_signature1 = func_signature1,
            elitist_code = filter_code(base_candidate["code"]),
            func_name = self.func_name,
        )
        messages = [{"role": "system", "content": system}, {"role": "user", "content": user}]
    ...
    def evolve(self):
        while self.function_evals < self.cfg.max_fe:
            ...
            # Select
            population_to_select = self.population if (not self.pareto_front or any(ind not in self.population for ind in self.pareto_front)) else self.pareto_front + self.population
            selected_population = self.MEoH_select(population_to_select)
        ...
        return self.pareto_front_code, self.pareto_front_code_path
